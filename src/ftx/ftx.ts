import { WebsocketClient, RestClient, OrderType, OrderSide } from 'ftx-api'
...
export class ftxCanisterWrapper {
  ...
  setClosePrice(closePrice: number) {
  ...
  _placeOrder = async (_params: {
    ...
    if (success) {
      sendMessage(
        `placed ${result.side} order, ${result.id}, of size ${result.size} at ${result.price}`,
      )
      return {
        id: result.id,
  ...
  placeConditionalOrders = async (params: {
    ...
    while (future) {
      ...
      const newPrice: any = await this._client.getFuture(params.market)
      const last_price: any = newPrice?.result.last
      try {
        params.size = params.size / last_price
        await sleep(2500)
        console.log('Bot Started and live price is:', last_price)

        const calc1Above = future - params.price
        const calc2Above = params.price - future
        ...
        if (calc1Above <= params.trigger && calc2Above <= params.trigger) {
          params.price =
            params.side === 'buy'
              ? last_price - params.chase?.by_amount
              : last_price + params.chase?.by_amount
          let message = ''
          message = `Target Price is Hit: \`${last_price}\`, Target Diffrence: ${
